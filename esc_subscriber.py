...
class servo_subscriber_node(Node):
    def __init__(self):
        super().__init__("subscriber_ESC")

        # inputs are message_type, topic name, callback function, qos profile {buffer size}
        self.subscriber_ = self.create_subscription(
            String, 'ESC_pwm', self.callback,100)  # creating subscriber on robot_news topic

        # assigning the logger to inform that subscriber is started
        self.get_logger().info('Subscriber node for ESC has been started.')
        
    def callback(self, msg):
        """

        This function subscribe to ESC_pwm topic to listen every published message 

        Parameters
        ----------
        msg : type of message
            message sent to ESC_pwm topic.
